...
export default function Tourney() {
  const [results, setResults] = useState<{ [team: number]: [number, number, number, number] }>();
  const [state, setState] = useState<State>(State.GROUP);
  useEffect(() => {
    defaultProvider.callContract({
      contractAddress: "0x0450a472cc83e3dc27529e7aef93e3893d9aa16a86fe8b6d49fd460f0537b408",
      entrypoint: "results",
      calldata: ["48", ...Array.from(Array(48).keys()).map(k => "" + (k + 1))],
    }).then(response => {
            ...
            {state === State.GROUP && (
              <MotionFlex direction="column" gap="40px">
                {Object.keys(TournamentData.groups).map((letter, idx) => {
                  ...
                  return <Group letter={letter} key={idx} teams={teams} results={results} />;
                })}
              </MotionFlex>
            )}
